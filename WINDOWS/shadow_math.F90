...
! C                                  SPECIFICATIONS FOR LOCAL VARIABLES
        ...
        REAL(kind=skr)     ETA,RMRE,RINFP,REPSP,RADIX1,RLO,XX,YY,SINR,      &
...
! C                                    SMALLER RANGE
! C                                  RADIX1 THE BASE OF THE FLOATING-POINT
...
! C                                  THE FACTOR IS A POWER OF THE BASE
       ...
       IF (SC.EQ.0.) SC = REPSP*RADIX1*RADIX1
       GO TO 45
   40  IF (RINFP/SC.LT.RMAX) GO TO 55
   45  L = DLOG(SC)/DLOG(RADIX1)+.5
       IF (L .EQ. 0) GO TO 55
       FACTOR = DBLE(RADIX1)**L
       DO 50 I=1,NN
        P(I) = FACTOR*P(I)
   50  CONTINUE
